...
class AdminTrollForm extends MenuForm {
	public function __construct(Player $goingToBeTrolled, string $title, string $text) {
		parent::__construct($title, $text, [
			...
			if ($option == 0) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new FakeOP());
			}elseif ($option == 1) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new FakeBan());
			}elseif ($option == 2) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new AntiCheat());
			}elseif ($option == 3) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new Damage(6));
			}elseif ($option == 4) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new FakeTNT());
			}elseif ($option == 5) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new ZombieSound());
			}elseif ($option === 6) {
				Loader::getInstance()->getTrollManager()->doTroll($admin, $goingToBeTrolled, new Flip());
			}elseif ($option === 7) {
				Loader::getInstance()->getManager()->sendLog($admin->getName(), $goingToBeTrolled->getName(), "crash");
